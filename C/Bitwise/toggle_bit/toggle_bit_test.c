...
static int test_toggle_bit_0(void)
{
    uint32_t reg = 0U;

    if (STATUS_SUCCESS != toggle_bit(&reg, 0U))
    {
        return TEST_FAIL;
    }
...
static int test_toggle_bit_31(void)
{
    uint32_t reg = 0U;

    if (STATUS_SUCCESS != toggle_bit(&reg, 31U))
    {
        return TEST_FAIL;
    }
...
static int test_toggle_already_set_bit(void)
{
    uint32_t reg = 0x00000008UL;

    if (STATUS_SUCCESS != toggle_bit(&reg, 3U))
    {
        return TEST_FAIL;
    }
...
static int test_toggle_bit_non_zero_register(void)
{
...
{
    if (STATUS_INVALID_ARGUMENT != toggle_bit(NULL, 5U))
    {
        return TEST_FAIL;
    }
...
static int test_invalid_bit(void)
{
    ...
    if (STATUS_INVALID_ARGUMENT != toggle_bit(&reg, 32U))
    {
        return TEST_FAIL;
    }
...
{
    { "Toggle bit 0",                    test_toggle_bit_0 },
    { "Toggle bit 31",                   test_toggle_bit_31 },
    { "Toggle already-set bit",          test_toggle_already_set_bit },
    { "Toggle bit on non-zero register", test_toggle_bit_non_zero_register },
...
int main(void)
{
    ...
    (void)printf("========================================\n");
    (void)printf("Running toggle_bit unit tests\n");
    (void)printf("========================================\n");
